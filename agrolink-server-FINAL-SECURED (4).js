...
async function migrateEncryptSensitiveColumns() {
    ...
    try {
        ...
        // Messages content: pgcrypto şifrelemesi mesajlarda KULLANILMIYOR
        // Sebep: Socket.IO + REST API karışık insert, tüm okuma noktaları güncellenmeli
        // Mesaj güvenliği JWT auth + HTTPS transport ile sağlanıyor
        // Eğer daha önce şifrelendiyse geri decrypt et (rollback)
        if (DB_ENCRYPTION_KEY) {
            await pool.query(`
                UPDATE messages
                SET content = pgp_sym_decrypt(content::bytea, $1)::text
                WHERE content IS NOT NULL
                  AND content LIKE '\xc0%'
            `, [DB_ENCRYPTION_KEY]).catch(e => {
                // Zaten plain text ise hata vermez, sessizce geç
                if (!e.message.includes('invalid') && !e.message.includes('decrypt')) {
                    console.warn('Messages decrypt rollback:', e.message);
                }
            });
            console.log('✅ [ŞİFRELEME] Mesajlar plain text formatına geri alındı');
        }
